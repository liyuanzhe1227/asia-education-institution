...
export function ConsultationForm() {
  ...
  return <form className="consultation-form" onSubmit={submit}>
    ...
    <div><label>{tr('咨询身份', 'Enquirer type')}<select name="audience" required defaultValue=""><option value="" disabled>{tr('请选择', 'Select')}</option><option>{tr('学生 / 家长', 'Student / Parent')}</option><option>{tr('研究者 / 职场人士', 'Researcher / Professional')}</option><option>{tr('院校 / 企业 / 公共机构', 'University / Company / Public body')}</option></select></label><label>{tr('关注服务', 'Service')}<select name="service" required defaultValue=""><option value="" disabled>{tr('请选择', 'Select')}</option><option>{tr('升学与申请', 'Admissions')}</option><option>{tr('学术与研究', 'Academic & Research')}</option><option>{tr('职业发展', 'Career Development')}</option><option>{tr('签证 / 永居 / 人才 / 公民身份', 'Visa / Residence / Talent / Citizenship')}</option><option>{tr('国际游学 / 实习', 'Study Programme / Internship')}</option><option>{tr('院校合作 / 机构研修', 'Partnership / Institutional Training')}</option></select></label></div>
