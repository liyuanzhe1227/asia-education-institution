...
export function ConsultationForm() {
  ...
  const audienceOptions = [
    tr('中国家庭 / 学生 / 家长', 'China-based Family / Student / Parent'),
    tr('研究者 / 职场人士', 'Researcher / Professional'),
    tr('院校 / 企业 / 公共机构', 'University / Company / Public body'),
  ];
  const serviceOptions = [
    tr('幼儿园 / 小学 / 初高中', 'Early Years / Primary / Secondary'),
    tr('本科 / 硕士 / 博士申请', 'Undergraduate / Master’s / Doctoral Admissions'),
    tr('机器人 / 数学 / 科研竞赛', 'Robotics / Mathematics / Research Competitions'),
    tr('学术与研究', 'Academic & Research'),
    tr('论文 / 专著 / 出版项目', 'Papers / Monographs / Publishing'),
  ...
  return <form className="consultation-form" onSubmit={submit}>
    ...
    <label>{tr('当前所在城市与意向国家或地区（如适用）', 'Current city and intended destination, if applicable')}<input name="destination" placeholder={tr('例如：成都；意向新加坡/英国；或“尚未确定”', 'For example: Chengdu; considering Singapore/UK; or “not decided”')} /></label>
    <label>{tr('需求说明', 'Message')}<textarea name="message" rows={5} required placeholder={tr('请说明学生年级或当前阶段、目标、时间、课程体系和希望解决的问题。请勿提交证件号码、完整成绩单或其他敏感文件。', 'Please describe the student’s year or current stage, goals, timing, curriculum and key questions. Do not submit ID numbers, full transcripts or other sensitive files.')} /></label>
