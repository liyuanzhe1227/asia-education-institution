  ...
  return (
    <footer className="lux-footer">
      ...
      <div className="footer-links">
        ...
        <div><small>{lang === 'zh' ? '合作与机构' : 'Institutional'}</small><Link href="/partnerships/">{lang === 'zh' ? '院校合作' : 'Partnerships'}</Link><Link href="/about/">{lang === 'zh' ? '关于 AEI' : 'About AEI'}</Link><a href="mailto:eric2015aei@gmail.com">{lang === 'zh' ? '电子邮件' : 'Email'}</a></div>
      </div>
      <div className="footer-legal">
        ...
        <small>© {new Date().getFullYear()} {lang === 'zh' ? '亚瑟（新加坡）教育学院 · 新加坡' : 'Asia Education Institution · Singapore'}</small>
      </div>
    </footer>
  );
}
