...
export function DetailPageTemplate({ data }: { data: DetailPageData }) {
  ...
  return <main>
    <section className={`detail-hero ${data.institutional ? 'institutional' : ''}`}>
      ...
      <div><strong>{l(data.proposition)}</strong><span>{l(data.intro)}</span><a href="/about/#consultation">{tr('预约初步沟通', 'Request an initial consultation')} ↗</a></div>
    </section>
    ...
    <section className="detail-process"><div><p>{tr('服务流程', 'DELIVERY PROCESS')}</p><h2>{tr('从诊断到复盘，\n每一步都有明确交付。', 'From diagnosis to review,\nevery stage has a clear deliverable.')}</h2></div><div>{data.process.map((item, i) => <article key={item.title.zh}><span>0{i + 1}</span><h3>{l(item.title)}</h3><p>{l(item.body)}</p></article>)}</div></section>
    ...
    <section className="detail-cases"><div className="detail-section-heading"><p>{tr('相关记录', 'RELATED RECORDS')}</p><h2>{tr('把方法放进真实情境', 'Methods in real contexts')}</h2></div><div>{data.relatedCases.map((item) => <a href={item.href} key={item.title.zh}><small>{l(item.meta)}</small><h3>{l(item.title)}</h3><span>{tr('查看记录', 'View record')} ↗</span></a>)}</div></section>
    ...
    <section className="page-cta"><p>{tr('需要进一步确认范围？', 'Need to confirm the right scope?')}</p><h2>{tr('从一次目标清晰的沟通开始。', 'Start with a focused conversation.')}</h2><a href="/about/#consultation">{tr('提交咨询信息', 'Submit an enquiry')} ↗</a></section>
  </main>;
}
