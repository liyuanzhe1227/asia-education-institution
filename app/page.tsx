  ...
  return (
    <main>
      ...
      <section className="editorial-section data-section">
        <div className="editorial-heading"><p>03 / {tr('年度项目案例', 'ANNUAL PROJECT CASES')}</p><h2>{tr('一个文件夹对应一个项目，\n年度积累一目了然', 'One folder, one project.\nA clear view by year.')}</h2><p className="heading-note">{tr('数字按“申请与服务归档”中的一级项目文件夹统计。档案主体为已成功完成或已形成明确结果的项目，少量为在办、关闭或单项交付。', 'Figures count first-level project folders in the application and service archive. Most projects were successfully completed or reached a documented outcome; a small number remain in progress, closed or limited to a single deliverable.')}</p></div>
        ...
        <div className="period-totals"><div><strong>16</strong><span>2016—2019</span></div><div><strong>47</strong><span>2020—2022</span></div><div><strong>63</strong><span>2023—2026</span></div><div className="total"><strong>126</strong><span>{tr('项目案例合计', 'TOTAL PROJECT CASES')}</span></div></div>
      </section>
