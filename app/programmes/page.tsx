...
export default function ProgrammesPage() {
  ...
  return <main>
    ...
    <section className="programme-track-list">{programmeTracks.map((track, i) => <article id={['immersion', 'academic', 'internship', 'executive'][i]} key={track.number}><div><span>{track.number}</span><small>{lang === 'zh' ? track.duration.zh : track.duration.en}</small></div><h2>{lang === 'zh' ? track.title.zh : track.title.en}</h2><p>{lang === 'zh' ? track.body.zh : track.body.en}</p><b>↗</b></article>)}</section>
    <section className="programme-design"><div><p>{tr('项目结构', 'PROGRAMME ARCHITECTURE')}</p><h2>{tr('五层项目结构', 'Five-layer programme architecture')}</h2></div><ol>{[[tr('主题', 'Theme'), tr('人工智能、可持续发展、科学技术工程与数学、商业创新或专业主题', 'AI, sustainability, STEM, business innovation or a specialist theme')],[tr('学习', 'Learning'), tr('讲座、案例、工作坊与团队任务', 'Lectures, cases, workshops and team assignments')],[tr('场景', 'Context'), tr('校园、实验室、企业与城市参访', 'Campus, laboratory, company and city visits')],[tr('产出', 'Output'), tr('展示、报告、反思与项目证明', 'Presentation, report, reflection and completion evidence')],[tr('保障', 'Assurance'), tr('筛选、行前、接待、风险与复盘', 'Selection, preparation, hosting, risk and review')]].map(([title, body], i) => <li key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{body}</p></li>)}</ol></section>
  </main>;
}
